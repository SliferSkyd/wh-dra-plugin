...
type Pool struct {
	...
	// TotalSliceCount overrides ResourceSliceCount when non-zero. Use this for
	// multi-node pools where several nodes each publish one slice but the total
	// expected count is greater than the number of slices this node contributes.
	// Setting TotalSliceCount=N on every node's slice means the scheduler will
	// not allocate from the pool until it can see N slices across all nodes.
