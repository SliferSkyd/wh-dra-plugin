...
type Pool struct {
	...
	// TotalSliceCount overrides ResourceSliceCount when non-zero. Use for
	// multi-node pools where each node contributes one slice but the total
	// expected count exceeds what any single node publishes.
